    ...
    class TaskWorker
      ...
      class << self
        ...
        def sidekiq_task_model(model_klass)
          ...
          get_sidekiq_task_options[:model_class] = parse_task_model_class(model_klass)
        end
        ...
        def parse_task_model_class(model_klass)
          begin
            model_klass = model_klass.to_s.classify.constantize
            raise unless model_klass <= task_model_base_class
          rescue
            fail ArgumentError.new "`sidekiq_task_model` must be an #{task_model_base_class.to_s} model"
          end
          model_klass
        end
